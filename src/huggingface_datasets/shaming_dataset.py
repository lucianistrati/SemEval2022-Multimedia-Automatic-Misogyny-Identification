...
class ShamingDataset(datasets.GeneratorBasedBuilder):
    def _info(self):
        labels = [0, 1]
        return datasets.DatasetInfo(
            ...
            citation=_CITATION,
            task_templates=[
                TextClassification(text_column="text", label_column="shaming_label",
                                   labels=labels)],
        )
        ...
        with open(filepath, encoding="utf-8") as csv_file:
            ...
            for id_, row in enumerate(csv_reader):
                ...
                if label not in labels:
                    continue
                yield id_, {"text": text, "shaming_label": label}